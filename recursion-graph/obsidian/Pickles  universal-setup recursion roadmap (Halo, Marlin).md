---
source_file: "references/recursion/ch3/ref-58-coda-mina.pdf"
type: "paper"
community: "Recursion Overview & History"
location: "§9 Future Work"
tags:
  - graphify/paper
  - graphify/INFERRED
  - community/Recursion_Overview__History
---

# Pickles / universal-setup recursion roadmap (Halo, Marlin)

## Connections
- [[Cited Halo (Bowe-Grigg-Hopwood) and Marlin (Chiesa et al.) — universal-setup recursion]] - `cites` [EXTRACTED]
- [[Mina (Coda)]] - `conceptually_related_to` [INFERRED]
- [[Recursive Proof Composition]] - `conceptually_related_to` [INFERRED]

#graphify/paper #graphify/INFERRED #community/Recursion_Overview__History