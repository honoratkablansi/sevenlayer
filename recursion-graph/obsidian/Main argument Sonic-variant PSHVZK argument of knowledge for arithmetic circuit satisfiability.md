---
source_file: "references/recursion/ch1/ref-05-halo.pdf"
type: "paper"
community: "Spartan, Halo & IVC Foundations"
location: "§5, §5.1, §5.2 (relation R, Fig. 1)"
tags:
  - graphify/paper
  - graphify/EXTRACTED
  - community/Spartan_Halo__IVC_Foundations
---

# Main argument: Sonic-variant PSHVZK argument of knowledge for arithmetic circuit satisfiability

## Connections
- [[Bulletproofs (Bunz et al.) 13]] - `conceptually_related_to` [EXTRACTED]
- [[Halo first practical recursive proof composition without a trusted setup]] - `introduces` [EXTRACTED]
- [[Nested amortization deferringcollapsing linear-time verification through recursive public inputs]] - `conceptually_related_to` [EXTRACTED]
- [[Non-interactive instantiation via Fiat-Shamir with Rescue duplex-sponge in random oracle model]] - `conceptually_related_to` [EXTRACTED]
- [[Pedersen-based univariate polynomial commitment with amortized succinctness]] - `assumes` [EXTRACTED]
- [[Sonic (Maller, Bowe, Kohlweiss, Meiklejohn) 32]] - `cites` [EXTRACTED]
- [[Theorem 2 main protocol (Fig. 1) has completeness, PSHVZK, computational witness-extended emulation]] - `proves` [EXTRACTED]

#graphify/paper #graphify/EXTRACTED #community/Spartan_Halo__IVC_Foundations