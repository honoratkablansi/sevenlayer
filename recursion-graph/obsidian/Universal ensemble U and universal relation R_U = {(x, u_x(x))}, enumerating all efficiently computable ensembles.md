---
source_file: "references/recursion/ch2/ref-51-canetti-goldreich-halevi-rom.pdf"
type: "paper"
community: "Fractal & Holographic Recursion"
location: "§4.2"
tags:
  - graphify/paper
  - graphify/EXTRACTED
  - community/Fractal__Holographic_Recursion
---

# Universal ensemble U and universal relation R_U = {(x, u_x(x))}, enumerating all efficiently computable ensembles

## Connections
- [[CGH separation technique feed the implementation's own codeseed back into the scheme via evasive relations; diagonalization over the enumeration of all poly-time ensembles to reverse quantifier order]] - `assumes` [EXTRACTED]
- [[Evasive binary relation (Def 3.1) infeasible to find x with (x,O(x)) in R under a random oracle]] - `conceptually_related_to` [EXTRACTED]

#graphify/paper #graphify/EXTRACTED #community/Fractal__Holographic_Recursion