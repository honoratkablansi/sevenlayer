---
type: community
cohesion: 0.06
members: 41
---

# zkVMs & Real-Time Proving

**Cohesion:** 0.06 - loosely connected
**Members:** 41 nodes

## Members
- [[BLS signatures]] - document - references/recursion/ch1/ref-37-ethereum-lean-hash-based-sig-aggregation.md
- [[Continuations]] - document - recursion/recursion-outline.md
- [[EXECUTE Precompile for Native zk-Rollups]] - document - references/recursion/ch1/ref-19-ef-l1-zkevm-realtime-proving.md
- [[Enshrined proofs (phase 3 of proving roadmap)]] - document - references/recursion/ch3/ref-57-ethproofs-reporting.md
- [[EthProofs Dashboard]] - document - references/recursion/ch1/ref-08-sp1-hypercube.md
- [[Hash-based cryptography (lean foundation)]] - document - references/recursion/ch1/ref-37-ethereum-lean-hash-based-sig-aggregation.md
- [[Hash-based signatures]] - document - references/recursion/ch1/ref-37-ethereum-lean-hash-based-sig-aggregation.md
- [[Home Proving]] - document - references/recursion/ch1/ref-19-ef-l1-zkevm-realtime-proving.md
- [[Jagged Polynomial Commitment Scheme]] - document - references/recursion/ch1/ref-08-sp1-hypercube.md
- [[L1 zkEVM]] - document - references/recursion/ch1/ref-19-ef-l1-zkevm-realtime-proving.md
- [[LogUp GKR Sumcheck Protocol]] - document - references/recursion/ch1/ref-08-sp1-hypercube.md
- [[Multilinear Polynomials]] - document - references/recursion/ch1/ref-08-sp1-hypercube.md
- [[Plonky3]] - document - references/recursion/ch1/ref-08-sp1-hypercube.md
- [[Post-quantum cryptography]] - document - references/recursion/ch1/ref-37-ethereum-lean-hash-based-sig-aggregation.md
- [[Proof of correct program execution]] - document - references/recursion/ch1/ref-28-risc-zero-technical-docs.md
- [[RISC Zero technical documentation (dev.risczero.com)]] - document - references/recursion/ch1/ref-28-risc-zero-technical-docs.md
- [[RISC Zero zkVM]] - document - references/recursion/ch1/ref-28-risc-zero-technical-docs.md
- [[RISC-V architecture]] - document - references/recursion/ch1/ref-27-succinct-sp1-docs.md
- [[RTP eligibility thresholds (sub-10s proofs, liveness, cost)]] - document - references/recursion/ch3/ref-57-ethproofs-reporting.md
- [[Real-time proving]] - document - recursion/recursion-outline.md
- [[Real-time zkVM pipeline case study]] - document - recursion/recursion-outline.md
- [[Real-time zkVMs (lean execution)]] - document - references/recursion/ch1/ref-37-ethereum-lean-hash-based-sig-aggregation.md
- [[Realtime Proving Standardized Definition]] - document - references/recursion/ch1/ref-19-ef-l1-zkevm-realtime-proving.md
- [[Receipt (self-certifying proof of execution)]] - document - references/recursion/ch1/ref-28-risc-zero-technical-docs.md
- [[Recursion (proof recursion in zkVMs)]] - document - references/recursion/ch1/ref-27-succinct-sp1-docs.md
- [[Rise of SNARKs]] - document - references/recursion/ch1/ref-37-ethereum-lean-hash-based-sig-aggregation.md
- [[SNARK-aggregated signatures]] - document - recursion/recursion-outline.md
- [[SP1 Hypercube]] - document - references/recursion/ch1/ref-08-sp1-hypercube.md
- [[SP1 Hypercube Proving Ethereum in Real-Time (Succinct, 2025)]] - document - references/recursion/ch1/ref-08-sp1-hypercube.md
- [[SP1 zkVM]] - document - references/recursion/ch1/ref-08-sp1-hypercube.md
- [[Security sprint milestone M2 (100-bit provable  600 KiB)]] - document - references/recursion/ch3/ref-57-ethproofs-reporting.md
- [[Shard proofs (SP1 proof system)]] - document - references/recursion/ch1/ref-27-succinct-sp1-docs.md
- [[Shipping an L1 zkEVM 1 Realtime Proving (Ethereum Foundation, 2025)]] - document - references/recursion/ch1/ref-19-ef-l1-zkevm-realtime-proving.md
- [[Stateless Proof Verification]] - document - references/recursion/ch1/ref-19-ef-l1-zkevm-realtime-proving.md
- [[Succinct SP1 technical documentation]] - document - references/recursion/ch1/ref-27-succinct-sp1-docs.md
- [[Succinct verification without re-execution]] - document - references/recursion/ch1/ref-28-risc-zero-technical-docs.md
- [[Verifiable computation]] - document - references/recursion/ch1/ref-28-risc-zero-technical-docs.md
- [[ethproofs.org real-time proving reporting (Pico Prism, ethrex)]] - document - references/recursion/ch3/ref-57-ethproofs-reporting.md
- [[lean Ethereum (Justin Drake, EF blog)]] - document - references/recursion/ch1/ref-37-ethereum-lean-hash-based-sig-aggregation.md
- [[zkVM]] - document - recursion/recursion-outline.md
- [[zkVM prover benchmarks (zkDTVM, OpenVM, ZisK, Pico on GPU clusters)]] - document - references/recursion/ch3/ref-57-ethproofs-reporting.md

## Live Query (requires Dataview plugin)

```dataview
TABLE source_file, type FROM #community/zkVMs__Real-Time_Proving
SORT file.name ASC
```

## Connections to other communities
- 3 edges to [[_COMMUNITY_Proof Aggregation & Recursion Basics]]
- 2 edges to [[_COMMUNITY_Recursion Overview & History]]
- 1 edge to [[_COMMUNITY_IVC Foundations (Valiant)]]
- 1 edge to [[_COMMUNITY_SuperNova vs Universal Circuits]]
- 1 edge to [[_COMMUNITY_STARK Recursion & Taxonomy]]
- 1 edge to [[_COMMUNITY_SuperNova Non-Uniform IVC]]
- 1 edge to [[_COMMUNITY_Rollups & Strategy Framework]]
- 1 edge to [[_COMMUNITY_Spartan, Halo & IVC Foundations]]
- 1 edge to [[_COMMUNITY_Fiat-Shamir & Random-Oracle Model]]
- 1 edge to [[_COMMUNITY_Ethereum zkEVM Roadmap]]

## Top bridge nodes
- [[zkVM]] - degree 8, connects to 4 communities
- [[Real-time proving]] - degree 9, connects to 2 communities
- [[SNARK-aggregated signatures]] - degree 4, connects to 2 communities
- [[SP1 Hypercube]] - degree 11, connects to 1 community
- [[L1 zkEVM]] - degree 6, connects to 1 community