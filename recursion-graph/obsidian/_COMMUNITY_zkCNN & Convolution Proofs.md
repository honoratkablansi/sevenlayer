...
```dataview
TABLE source_file, type FROM #community/zkCNN__Convolution_Proofs
SORT file.name ASC
```

## Connections to other communities
- 6 edges to [[_COMMUNITY_Sum-Check & CCS SNARKs]]
- 6 edges to [[_COMMUNITY_Applications Cross-Chain & zkML]]
- 3 edges to [[_COMMUNITY_GKR & Delegation (Muggles)]]
- 3 edges to [[_COMMUNITY_Fiat-Shamir Attacks]]
- 2 edges to [[_COMMUNITY_Polynomial-Commitment Accumulation]]
- 2 edges to [[_COMMUNITY_Verifiable Databases (vSQL)]]
- 2 edges to [[_COMMUNITY_Fiat-Shamir & Random-Oracle Model]]

## Top bridge nodes
- [[GKR protocol doubly-efficient interactive proof for bounded-depth computation via layer-by-layer sumcheck reduction; prover need not commit to full trace]] - degree 19, connects to 6 communities
- [[zkCNN commit-and-prove scheme (KeyGen, Commit, Prove, Verify) for CNN prediction]] - degree 11, connects to 3 communities
- [[zkCNN Zero Knowledge Proofs for CNN Predictions and Accuracy (CCS 2021)]] - degree 8, connects to 2 communities
- [[2-D convolution via 1-D convolution = IFFT(FFT(X) (.) FFT(W)); three sum-checks (FFT, Hadamard, IFFT), prover O(n2)]] - degree 3, connects to 1 community
