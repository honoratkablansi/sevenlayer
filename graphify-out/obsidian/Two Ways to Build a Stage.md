---
source_file: "wiki/sections/ch02-two-ways-to-build-a-stage.md"
type: "document"
community: "Community 0"
tags:
  - graphify/document
  - graphify/EXTRACTED
  - community/Community_0
---

# Two Ways to Build a Stage

## Connections
- [[Ariel Gabizon]] - `conceptually_related_to` [EXTRACTED]
- [[BN254 Curve]] - `conceptually_related_to` [EXTRACTED]
- [[Bulletproofs]] - `conceptually_related_to` [EXTRACTED]
- [[FRI]] - `conceptually_related_to` [EXTRACTED]
- [[Folding Schemes]] - `conceptually_related_to` [EXTRACTED]
- [[Groth16]] - `conceptually_related_to` [EXTRACTED]
- [[Halo]] - `conceptually_related_to` [EXTRACTED]
- [[Inner Product Arguments (IPA)]] - `conceptually_related_to` [EXTRACTED]
- [[KZG Polynomial Commitments]] - `conceptually_related_to` [EXTRACTED]
- [[Layer 1 -- Building the Stage]] - `part_of` [EXTRACTED]
- [[Nova]] - `conceptually_related_to` [EXTRACTED]
- [[PLONK]] - `conceptually_related_to` [EXTRACTED]
- [[STARKs]] - `conceptually_related_to` [EXTRACTED]
- [[The 141,416-Person Question]] - `references` [EXTRACTED]
- [[The ADOPT Framework]] - `references` [EXTRACTED]
- [[The CapexOpex Framework]] - `references` [EXTRACTED]
- [[The Fair Shuffle Problem]] - `references` [EXTRACTED]
- [[The Quantum Shelf Life]] - `references` [EXTRACTED]
- [[The Structured Reference String]] - `references` [EXTRACTED]
- [[Trusted Setup Ceremony]] - `conceptually_related_to` [EXTRACTED]
- [[Zcash]] - `conceptually_related_to` [EXTRACTED]

#graphify/document #graphify/EXTRACTED #community/Community_0