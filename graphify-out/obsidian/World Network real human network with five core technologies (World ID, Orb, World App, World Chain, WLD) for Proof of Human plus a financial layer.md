---
source_file: "references/ch13/ref-59-world-whitepaper.pdf"
type: "paper"
community: "World & Proof of Personhood"
location: "§1.2.2, §1.2.5"
tags:
  - graphify/paper
  - graphify/EXTRACTED
  - community/World__Proof_of_Personhood
---

# World Network: real human network with five core technologies (World ID, Orb, World App, World Chain, WLD) for Proof of Human plus a financial layer

## Connections
- [[Proof of Human (PoH) private primitive establishing a party is a real, unique human (or agent acting on a human's behalf)]] - `introduces` [EXTRACTED]
- [[Proof of Personhood (Proof of Human)]] - `conceptually_related_to` [EXTRACTED]
- [[World Chain human-centered financial layer implemented as an OP Stack  Superchain layer-2 on Ethereum with human-based transaction prioritization]] - `defines` [EXTRACTED]
- [[World ID open-source decentralized privacy-preserving digital identity issuing unlinkable proofs (Proof of Human, Age, Country) via personal custody, AMPC and ZKPs]] - `defines` [EXTRACTED]
- [[World Whitepapers (World Network Proof of Human)]] - `defines` [EXTRACTED]
- [[Worldcoin (WLD) ERC-20 native token, 10B initial cap, =60% target to verified humans as user tokens, governanceone-person-one-vote, =1.5% inflation only after 15 years]] - `defines` [EXTRACTED]

#graphify/paper #graphify/EXTRACTED #community/World__Proof_of_Personhood