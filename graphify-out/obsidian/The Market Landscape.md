---
source_file: "wiki/chapters/13-the-market-landscape.md"
type: "document"
community: "Coprocessors & ZKML"
tags:
  - graphify/document
  - graphify/EXTRACTED
  - community/Coprocessors__ZKML
---

# The Market Landscape

## Connections
- [[Circle STARKs]] - `references` [EXTRACTED]
- [[DTCC and Digital Asset Partner to Tokenize DTC-Custodied U.S. Treasury Securities on the Canton Network]] - `references` [EXTRACTED]
- [[Enterprise Pilots (Pilot)]] - `part_of` [EXTRACTED]
- [[Ethproofs ZK Proving Cost Tracker (Ethereum Foundation)]] - `references` [EXTRACTED]
- [[FRI]] - `references` [EXTRACTED]
- [[Fiat-Shamir Transform]] - `references` [EXTRACTED]
- [[Groth16]] - `references` [EXTRACTED]
- [[Halo 2]] - `references` [EXTRACTED]
- [[KZG Polynomial Commitments]] - `references` [EXTRACTED]
- [[L2BEAT Stages Framework for L2 Maturity]] - `cites` [EXTRACTED]
- [[Market Sizing]] - `part_of` [EXTRACTED]
- [[Midnight]] - `references` [EXTRACTED]
- [[PLONK]] - `references` [EXTRACTED]
- [[Privacy Pools (0xbow)]] - `references` [EXTRACTED]
- [[Proving Nothing]] - `part_of` [EXTRACTED]
- [[Proving-as-a-Service]] - `references` [EXTRACTED]
- [[Proving-as-a-Service The Prover Market (Production)]] - `part_of` [EXTRACTED]
- [[RISC Zero]] - `references` [EXTRACTED]
- [[Regulation (EU) 20241183 establishing the European Digital Identity Framework (eIDAS 2)]] - `cites` [EXTRACTED]
- [[SP1  SP1 Hypercube]] - `references` [EXTRACTED]
- [[Selective Disclosure]] - `references` [EXTRACTED]
- [[Sevenlayer Wiki Index]] - `references` [EXTRACTED]
- [[Stwo]] - `references` [EXTRACTED]
- [[The Economics of ZK-Proving Market Size and Future Projections (Chorus One)]] - `references` [EXTRACTED]
- [[Tornado Cash]] - `references` [EXTRACTED]
- [[Trust Decomposition]] - `references` [EXTRACTED]
- [[Trusted Setup]] - `references` [EXTRACTED]
- [[World Whitepapers (World Network Proof of Human)]] - `cites` [EXTRACTED]
- [[ZK Coprocessors]] - `references` [EXTRACTED]
- [[ZK Coprocessors Off-Chain Computation, On-Chain Verification (Growth)]] - `part_of` [EXTRACTED]
- [[ZK Identity]] - `references` [EXTRACTED]
- [[ZK Identity (Growth  Regulatory Mandate)]] - `part_of` [EXTRACTED]
- [[ZK Proofs Is Privacy Cheap Enough to Be Mainstream (Castle Labs)]] - `references` [EXTRACTED]
- [[ZK Rollups]] - `references` [EXTRACTED]
- [[ZK Rollups_1]] - `references` [EXTRACTED]
- [[ZK Rollups The Proving Grounds (Production)]] - `part_of` [EXTRACTED]
- [[ZKML (Provable Machine Learning)]] - `references` [EXTRACTED]
- [[ZKML Provable Machine Learning (Research)]] - `part_of` [EXTRACTED]
- [[Zero-Knowledge Proof Market Size, Share & Trends Analysis Report (Grand View Research)]] - `references` [EXTRACTED]
- [[eIDAS 2.0_1]] - `references` [EXTRACTED]
- [[zkEVM]] - `references` [EXTRACTED]

#graphify/document #graphify/EXTRACTED #community/Coprocessors__ZKML