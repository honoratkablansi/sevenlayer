---
source_file: "references/ch02/ref-06-groth16.pdf"
type: "paper"
community: "Community 3"
tags:
  - graphify/paper
  - graphify/EXTRACTED
  - community/Community_3
---

# On the Size of Pairing-Based Non-interactive Arguments

## Connections
- [[3-Element Pairing-based SNARK for Arithmetic Circuit SAT]] - `introduces` [EXTRACTED]
- [[Bilinear Pairings]] - `conceptually_related_to` [EXTRACTED]
- [[Bitansky-Chiesa-Ishai-Ostrovsky-Paneth SNARKs via Linear Interactive Proofs (BCI+13)]] - `cites` [EXTRACTED]
- [[Complete Bibliography]] - `cites` [EXTRACTED]
- [[Danezis-Fournet-Groth-Kohlweiss Square Span Programs (DFGK14)]] - `cites` [EXTRACTED]
- [[Disclosure-free Common Reference String]] - `introduces` [EXTRACTED]
- [[Gennaro-Gentry-Parno-Raykova QSPQAP and Succinct NIZKs (GGPR13)]] - `cites` [EXTRACTED]
- [[Gentry-Wichs SNARGs Need Non-falsifiable Assumptions (GW11)]] - `cites` [EXTRACTED]
- [[Groth16]] - `defines` [EXTRACTED]
- [[Jens Groth]] - `conceptually_related_to` [EXTRACTED]
- [[Layer 1 -- Building the Stage]] - `cites` [EXTRACTED]
- [[Layer 5 -- The Sealed Certificate]] - `cites` [EXTRACTED]
- [[PLONK Permutations over Lagrange-bases for Oecumenical Noninteractive arguments of Knowledge]] - `cites` [EXTRACTED]
- [[Parno-Howell-Gentry-Raykova Pinocchio (PHGR13)]] - `cites` [EXTRACTED]
- [[Quadratic Arithmetic Programs]] - `conceptually_related_to` [EXTRACTED]
- [[Recursive SNARK Composition (Preprocessing to Fully Succinct)]] - `conceptually_related_to` [EXTRACTED]
- [[SNARKs]] - `conceptually_related_to` [EXTRACTED]
- [[Scalable Multi-party Computation for zk-SNARK Parameters in the Random Beacon Model]] - `cites` [EXTRACTED]
- [[Shoup Lower Bounds for Discrete Logarithms (Generic Group Model) (Sho97)]] - `cites` [EXTRACTED]
- [[The Knowledge Complexity of Interactive Proof Systems]] - `cites` [EXTRACTED]
- [[The Promise of Provable and Programmable Secrets]] - `cites` [EXTRACTED]
- [[Theorem 3 No NILP Has a Degree-1 (Linear) Verifier]] - `proves` [EXTRACTED]
- [[Theorem 4 Pairing-based Arguments Need Elements in Both G1 and G2]] - `proves` [EXTRACTED]
- [[Zero-Knowledge Proofs_1]] - `conceptually_related_to` [EXTRACTED]

#graphify/paper #graphify/EXTRACTED #community/Community_3