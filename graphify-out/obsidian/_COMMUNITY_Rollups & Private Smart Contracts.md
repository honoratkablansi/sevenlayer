---
type: community
cohesion: 0.14
members: 14
---

# Rollups & Private Smart Contracts

**Cohesion:** 0.14 - loosely connected
**Members:** 14 nodes

## Members
- [[Aggelos Kiayias]] - paper - references/ch09/ref-30-kachina.pdf
- [[Blockchain Privacy and Regulatory Compliance Towards a Practical Equilibrium]] - paper - references/ch09/ref-31-blockchain-privacy-compliance.pdf
- [[Fraud Proof System]] - document - references/ch08/ref-27-l2beat-stages.md
- [[Kachina -- Foundations of Private Smart Contracts]] - paper - references/ch09/ref-30-kachina.pdf
- [[L2BEAT Stages Framework]] - document - references/ch08/ref-27-l2beat-stages.md
- [[L2BEAT Stages Framework for L2 Maturity]] - document - references/ch08/ref-27-l2beat-stages.md
- [[Merkle Tree]] - paper - references/ch09/ref-31-blockchain-privacy-compliance.pdf
- [[Non-Interactive Zero-Knowledge (NIZK)]] - paper - references/ch09/ref-30-kachina.pdf
- [[Privacy Pools (0xbow)]] - document - wiki/sections/ch13-zk-identity-growth-regulatory-mandate.md
- [[Private Smart Contracts]] - paper - references/ch09/ref-30-kachina.pdf
- [[Universal Composability (UC)]] - paper - references/ch09/ref-30-kachina.pdf
- [[Vitalik Buterin]] - document - references/ch08/ref-27-l2beat-stages.md
- [[ZK Rollups]] - document - wiki/chapters/13-the-market-landscape.md
- [[Zerocash]] - paper - references/ch09/ref-30-kachina.pdf

## Live Query (requires Dataview plugin)

```dataview
TABLE source_file, type FROM #community/Rollups__Private_Smart_Contracts
SORT file.name ASC
```

## Connections to other communities
- 3 edges to [[_COMMUNITY_Trusted Setup & SNARK Foundations]]
- 3 edges to [[_COMMUNITY_Privacy-Enhancing Technologies]]
- 3 edges to [[_COMMUNITY_Midnight Privacy Stack]]
- 3 edges to [[_COMMUNITY_Coprocessors & ZKML]]
- 2 edges to [[_COMMUNITY_Arithmetization & Proving Frontiers]]
- 2 edges to [[_COMMUNITY_Rollup Security, Attacks & Aggregation]]
- 2 edges to [[_COMMUNITY_ZK Market & Identity]]
- 1 edge to [[_COMMUNITY_STARKs & zkVM Landscape]]

## Top bridge nodes
- [[L2BEAT Stages Framework for L2 Maturity]] - degree 9, connects to 4 communities
- [[Blockchain Privacy and Regulatory Compliance Towards a Practical Equilibrium]] - degree 8, connects to 4 communities
- [[Kachina -- Foundations of Private Smart Contracts]] - degree 8, connects to 3 communities
- [[Privacy Pools (0xbow)]] - degree 5, connects to 3 communities
- [[Merkle Tree]] - degree 3, connects to 2 communities