---
source_file: "wiki/chapters/01-the-promise-of-provable-and-programmable-secrets.md"
type: "document"
community: "Community 0"
tags:
  - graphify/document
  - graphify/EXTRACTED
  - community/Community_0
---

# The Promise of Provable and Programmable Secrets

## Connections
- [[BLS12-381 Curve]] - `references` [EXTRACTED]
- [[BN254 Curve]] - `references` [EXTRACTED]
- [[Beanstalk]] - `references` [EXTRACTED]
- [[Circle STARKs]] - `references` [EXTRACTED]
- [[Completeness]] - `defines` [EXTRACTED]
- [[Ethproofs ZK Proving Cost Tracker (Ethereum Foundation)]] - `cites` [EXTRACTED]
- [[FRI]] - `references` [EXTRACTED]
- [[Fiat-Shamir Transform]] - `references` [EXTRACTED]
- [[Folding Schemes]] - `references` [EXTRACTED]
- [[Goldilocks Field]] - `references` [EXTRACTED]
- [[Groth16]] - `references` [EXTRACTED]
- [[How to Read This Guide]] - `part_of` [EXTRACTED]
- [[Interactive Proof Systems_1]] - `references` [EXTRACTED]
- [[Jolt zkVM]] - `references` [EXTRACTED]
- [[KZG Polynomial Commitments]] - `references` [EXTRACTED]
- [[Lattice Cryptography]] - `references` [EXTRACTED]
- [[Midnight]] - `references` [EXTRACTED]
- [[On the Size of Pairing-Based Non-interactive Arguments]] - `cites` [EXTRACTED]
- [[PLONK]] - `references` [EXTRACTED]
- [[Profiles of the Future (Arthur C. Clarke, 1962)]] - `references` [EXTRACTED]
- [[Proving Nothing]] - `part_of` [EXTRACTED]
- [[Quadratic Arithmetic Programs]] - `references` [EXTRACTED]
- [[Regulation (EU) 20241183 establishing the European Digital Identity Framework (eIDAS 2)]] - `cites` [EXTRACTED]
- [[SNARKs_1]] - `references` [EXTRACTED]
- [[SP1  SP1 Hypercube]] - `references` [EXTRACTED]
- [[SP1 Hypercube Proving Ethereum in Real-Time (Succinct Labs)]] - `cites` [EXTRACTED]
- [[STARKs]] - `references` [EXTRACTED]
- [[Selective Disclosure]] - `defines` [EXTRACTED]
- [[Seven-Layer ZK Stack Model]] - `defines` [EXTRACTED]
- [[Sevenlayer Wiki Index]] - `references` [EXTRACTED]
- [[SoK What Don't We Know Understanding Security Vulnerabilities in SNARKs]] - `cites` [EXTRACTED]
- [[Soundness]] - `defines` [EXTRACTED]
- [[Stwo]] - `references` [EXTRACTED]
- [[Sudoku running example]] - `references` [EXTRACTED]
- [[The Deepest Question]] - `part_of` [EXTRACTED]
- [[The First Decision]] - `part_of` [EXTRACTED]
- [[The Knowledge Complexity of Interactive Proof Systems]] - `cites` [EXTRACTED]
- [[The Phenomenon]] - `part_of` [EXTRACTED]
- [[The Proof at the Door]] - `part_of` [EXTRACTED]
- [[The Seven Layers at a Glance]] - `part_of` [EXTRACTED]
- [[The Trick]] - `part_of` [EXTRACTED]
- [[Three Converging Forces]] - `part_of` [EXTRACTED]
- [[Tornado Cash]] - `references` [EXTRACTED]
- [[Trusted Setup]] - `references` [EXTRACTED]
- [[Under-Constrained Circuits]] - `references` [EXTRACTED]
- [[ZK Proofs Is Privacy Cheap Enough to Be Mainstream (Castle Labs)]] - `cites` [EXTRACTED]
- [[ZK Rollups_1]] - `references` [EXTRACTED]
- [[Zcash]] - `references` [EXTRACTED]
- [[Zero-Knowledge Proofs_1]] - `defines` [EXTRACTED]

#graphify/document #graphify/EXTRACTED #community/Community_0