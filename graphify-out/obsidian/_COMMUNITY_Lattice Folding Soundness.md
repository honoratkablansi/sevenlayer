---
type: community
cohesion: 0.50
members: 4
---

# Lattice Folding Soundness

**Cohesion:** 0.50 - moderately connected
**Members:** 4 nodes

## Members
- [[B-Relaxed Binding of Ajtai Commitment]] - paper - references/ch06/ref-20-latticefold.pdf
- [[Strong Sampling Set]] - paper - references/ch06/ref-20-latticefold.pdf
- [[Theorem 3.3 Folding Knowledge Soundness]] - paper - references/ch06/ref-20-latticefold.pdf
- [[Theorem 4.3 Pi_cm is a RoK from R_rg,B to R_com]] - paper - references/ch02/ref-11-latticefold-plus.pdf

## Live Query (requires Dataview plugin)

```dataview
TABLE source_file, type FROM #community/Lattice_Folding_Soundness
SORT file.name ASC
```

## Connections to other communities
- 2 edges to [[_COMMUNITY_Lattice Folding (LatticeFold)]]
- 1 edge to [[_COMMUNITY_Sumcheck SNARKs (SuperSpartan)]]
- 1 edge to [[_COMMUNITY_Lattice Folding (Neo)]]
- 1 edge to [[_COMMUNITY_Lattice Folding (LatticeFold+)]]

## Top bridge nodes
- [[Theorem 4.3 Pi_cm is a RoK from R_rg,B to R_com]] - degree 3, connects to 2 communities
- [[Strong Sampling Set]] - degree 3, connects to 1 community
- [[B-Relaxed Binding of Ajtai Commitment]] - degree 3, connects to 1 community
- [[Theorem 3.3 Folding Knowledge Soundness]] - degree 2, connects to 1 community