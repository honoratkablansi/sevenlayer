---
type: community
cohesion: 0.08
members: 67
---

# Trusted Setup & SNARK Foundations

**Cohesion:** 0.08 - loosely connected
**Members:** 67 nodes

## Members
- [[Algebraic Group Model (AGM)]] - document - references/ch10/ref-38-snarky-ceremonies.pdf
- [[Ariel Gabizon]] - document - wiki/sections/ch02-the-bug-that-was-not-a-ceremony-failure.md
- [[BN254's Eroding Security Margin]] - document - wiki/sections/ch02-bn254-s-eroding-security-margin.md
- [[Complete Bibliography]] - document - wiki/BIBLIOGRAPHY.md
- [[Completeness]] - document - proving-nothing.md
- [[Elliptic Curve Cryptography]] - document - proving-nothing.md
- [[Groth16]] - document - wiki/chapters/01-the-promise-of-provable-and-programmable-secrets.md
- [[Halo]] - document - wiki/chapters/07-the-deep-craft.md
- [[How to Read This Guide]] - document - wiki/sections/ch01-how-to-read-this-guide.md
- [[Interactive Proof Systems_1]] - document - proving-nothing.md
- [[Interactive Proof Systems]] - document - references/ch01/ref-02-gmr-knowledge-complexity.pdf
- [[Jens Groth]] - document - wiki/sections/ch06-the-three-families.md
- [[Knowledge Complexity]] - document - references/ch01/ref-02-gmr-knowledge-complexity.pdf
- [[Layer 1 -- Building the Stage]] - document - wiki/chapters/02-building-the-stage.md
- [[Mary Maller]] - document - references/ch10/ref-36-sonic.pdf
- [[Midnight's BLS12-381 Stage]] - document - wiki/sections/ch02-midnight-s-bls12-381-stage.md
- [[Multi-Party Computation Setup Ceremony]] - document - references/ch10/ref-37-updatable-universal-crs.pdf
- [[NIST]] - document - wiki/sections/ch01-the-deepest-question.md
- [[On the Size of Pairing-Based Non-interactive Arguments]] - paper - references/ch02/ref-06-groth16.pdf
- [[Option-Value Analysis]] - document - wiki/sections/ch02-option-value-analysis.md
- [[PLONK]] - document - wiki/GLOSSARY.md
- [[PLONK Permutations over Lagrange-bases for Oecumenical Noninteractive arguments of Knowledge]] - paper - references/ch02/ref-07-plonk.pdf
- [[Permutation Argument]] - document - references/ch02/ref-07-plonk.pdf
- [[Powers-of-Tau Setup]] - document - references/ch02/ref-10-sok-powers-of-tau.pdf
- [[Profiles of the Future (Arthur C. Clarke, 1962)]] - document - references/ch01/ref-01-clarke-profiles-of-the-future.md
- [[Quadratic Arithmetic Programs]] - document - references/ch02/ref-06-groth16.pdf
- [[Random Beacon Model]] - document - references/ch02/ref-05-mpc-random-beacon.pdf
- [[SNARKs]] - document - wiki/sections/ch10-the-binary-that-broke.md
- [[SNARKs_1]] - document - proving-nothing.md
- [[Scalable Multi-party Computation for zk-SNARK Parameters in the Random Beacon Model]] - paper - references/ch02/ref-05-mpc-random-beacon.pdf
- [[Seven-Layer ZK Stack Model]] - document - proving-nothing.md
- [[Snarky Ceremonies]] - paper - references/ch10/ref-38-snarky-ceremonies.pdf
- [[SoK Trusted Setups for Powers-of-Tau Strings]] - paper - references/ch02/ref-10-sok-powers-of-tau.pdf
- [[SoK What Don't We Know Understanding Security Vulnerabilities in SNARKs]] - paper - references/ch01/ref-03-sok-snark-vulnerabilities.pdf
- [[Sonic]] - document - proving-nothing.md
- [[Sonic Zero-Knowledge SNARKs from Linear-Size Universal and Updatable Structured Reference Strings]] - paper - references/ch10/ref-36-sonic.pdf
- [[Soundness]] - document - proving-nothing.md
- [[Structured Reference String (SRS)]] - document - proving-nothing.md
- [[Sudoku running example]] - document - wiki/concepts/sudoku.md
- [[The 141,416-Person Question]] - document - wiki/sections/ch02-the-141-416-person-question.md
- [[The ADOPT Framework]] - document - wiki/sections/ch02-the-adopt-framework.md
- [[The Bug That Was Not a Ceremony Failure]] - document - wiki/sections/ch02-the-bug-that-was-not-a-ceremony-failure.md
- [[The CapexOpex Framework]] - document - wiki/sections/ch02-the-capex-opex-framework.md
- [[The Deepest Question]] - document - wiki/sections/ch01-the-deepest-question.md
- [[The Deepest Symmetry]] - document - wiki/sections/ch08-the-deepest-symmetry.md
- [[The Fair Shuffle Problem]] - document - wiki/sections/ch02-the-fair-shuffle-problem.md
- [[The First Decision]] - document - wiki/sections/ch01-the-first-decision.md
- [[The Knowledge Complexity of Interactive Proof Systems]] - paper - references/ch01/ref-02-gmr-knowledge-complexity.pdf
- [[The Phenomenon]] - document - wiki/sections/ch01-the-phenomenon.md
- [[The Promise of Provable and Programmable Secrets]] - document - wiki/chapters/01-the-promise-of-provable-and-programmable-secrets.md
- [[The Proof at the Door]] - document - wiki/sections/ch01-the-proof-at-the-door.md
- [[The Quantum Shelf Life]] - document - wiki/sections/ch02-the-quantum-shelf-life.md
- [[The Setup Tradeoff]] - document - wiki/sections/ch02-the-setup-tradeoff.md
- [[The Seven Layers at a Glance]] - document - wiki/sections/ch01-the-seven-layers-at-a-glance.md
- [[The Structured Reference String]] - document - wiki/sections/ch02-the-structured-reference-string.md
- [[The Trick]] - document - wiki/sections/ch01-the-trick.md
- [[Three Converging Forces]] - document - wiki/sections/ch01-three-converging-forces.md
- [[Toxic Waste (Setup Trapdoor)]] - document - proving-nothing.md
- [[Trusted Setup Ceremony]] - document - wiki/sections/ch02-two-ways-to-build-a-stage.md
- [[Two Ways to Build a Stage]] - document - wiki/sections/ch02-two-ways-to-build-a-stage.md
- [[Under-Constrained Circuits]] - document - wiki/chapters/03-choreographing-the-act.md
- [[Universal and Updatable Structured Reference String]] - document - references/ch02/ref-07-plonk.pdf
- [[Universal versus Circuit-Specific Setups]] - document - wiki/sections/ch02-universal-versus-circuit-specific-setups.md
- [[Updatable and Universal Common Reference Strings with Applications to zk-SNARKs]] - paper - references/ch10/ref-37-updatable-universal-crs.pdf
- [[Updatable and Universal Structured Reference String]] - document - references/ch10/ref-37-updatable-universal-crs.pdf
- [[Zcash]] - document - wiki/chapters/01-the-promise-of-provable-and-programmable-secrets.md
- [[Zero-Knowledge Proofs_1]] - document - references/ch01/ref-02-gmr-knowledge-complexity.pdf

## Live Query (requires Dataview plugin)

```dataview
TABLE source_file, type FROM #community/Trusted_Setup__SNARK_Foundations
SORT file.name ASC
```

## Connections to other communities
- 92 edges to [[_COMMUNITY_Commitments & Post-Quantum Craft]]
- 64 edges to [[_COMMUNITY_STARKs & zkVM Landscape]]
- 56 edges to [[_COMMUNITY_Arithmetization & Proving Frontiers]]
- 48 edges to [[_COMMUNITY_Midnight Privacy Stack]]
- 30 edges to [[_COMMUNITY_Rollup Security, Attacks & Aggregation]]
- 4 edges to [[_COMMUNITY_Pairings & Quantum Threat]]
- 3 edges to [[_COMMUNITY_Privacy-Enhancing Technologies]]
- 3 edges to [[_COMMUNITY_Rollups & Private Smart Contracts]]
- 2 edges to [[_COMMUNITY_Coprocessors & ZKML]]
- 1 edge to [[_COMMUNITY_Digital Identity & Personhood]]
- 1 edge to [[_COMMUNITY_ZK Market & Identity]]

## Top bridge nodes
- [[Layer 1 -- Building the Stage]] - degree 56, connects to 7 communities
- [[The Promise of Provable and Programmable Secrets]] - degree 49, connects to 7 communities
- [[Groth16]] - degree 67, connects to 6 communities
- [[PLONK]] - degree 51, connects to 6 communities
- [[Complete Bibliography]] - degree 16, connects to 6 communities