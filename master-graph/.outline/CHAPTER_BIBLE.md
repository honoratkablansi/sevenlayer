...
## D. Known gap flagged by the omissions analysis — RESOLVED (2026-06-14)

The graph-wide omissions pass found the outline captures ~98% of the high-degree god-nodes, with **one genuine load-bearing miss**: the **classical interactive-ZK / commitment-scheme on-ramp** (graph community C80) — Sigma protocols, Schnorr, the three-coloring and graph-isomorphism demonstrations, proof-of-knowledge, special soundness, and the generic **Commitment Scheme** (deg 17) / **Pedersen commitment** (deg 39, ref 14). Commitments were *used* downstream (IPA in Ch 11, accumulation in Ch 14) but never *built*.

**Resolution (folded in via the Feynman spiral — no chapters added or renumbered):**
- **Ch 2 (intuition on-ramp):** a light, apprenticeship-register interlude installs commitment schemes as the *sealed envelope* (hiding + binding), the **Sigma-protocol** commit-challenge-response template with **Schnorr** as the worked example, the **simulation paradigm** at intuition depth ("a secret-less simulator could fake the transcript, so nothing leaks"), and the canonical demonstrations — **graph 3-coloring** (under cups), **graph isomorphism / non-isomorphism**, and **quadratic residuosity**. The entry flags that the rigor lands later.
- **Ch 9–10 (formal lock-down):** commitment *hiding/binding* are defined rigorously (the PCS as the polynomial special case), and **special soundness** + **proof of knowledge / the knowledge extractor** are defined (Ch 9) and *exhibited on a real family* (Ch 10), tied back to Ch 2's Schnorr/Sigma "extract from two transcripts" move.
- **Ch 11 (construction):** the **Pedersen commitment** (`g^m h^r`, perfectly hiding / computationally binding, plus the vector form) is built as the simplest discrete-log commitment — closing the Ch 2 spiral — with **IPA shown as a Pedersen-vector argument**, and the **Discrete Logarithm Assumption** named explicitly as the hardness bet (alongside CDH/q-SDH).
- **Smaller named-but-absent nodes surfaced:** **Mina/Pickles** as the production-IVC exemplar (Ch 13), and **deVirgo / distributed (collaborative) proving** in the prover-market material (Ch 19).

These edits are reflected in the patched part files below and in the spec's Section 5 homes table. See the full appendix (`omissions.md`, reproduced at the end of this document) for the original rationale.
...
2. **Content walk.** The magician/audience metaphor is mapped onto prover/verifier. The Fiat-Shamir turn is previewed as a slogan — "fire the referee, hire a hash" — to plant non-interactivity without building it. The "why now?" section names the three converging forces: the privacy crisis, the scaling crisis, and the cost collapse (roughly $80 → $0.04 per proof). The seven trust bets are walked once, lightly, each with its scar attached (e.g., subverted setup, under-constrained circuit, side-channel leak, governance capture) so the reader leaves with a map and a sense of stakes. The chapter closes by opening the **debt ledger**: a visible promise that "why Fiat-Shamir is sound, why 192 bytes suffice, why the math can't be faked" will be built in Part III.
   ...
   **The classical on-ramp (an intuition interlude, kept light).** Before the seven-trust map, the chapter installs the historical scaffolding the magician metaphor has been standing on — at apprenticeship register, no heavy math, and with an explicit note that the rigor lands later. (a) **Commitment schemes** are introduced as a *sealed envelope*: you write a value on a slip, seal it, and hand it over; later you can open it, but you couldn't have changed it once sealed. The two properties are named in plain terms — *hiding* (the envelope conceals the value until you choose to open it) and *binding* (you can't swap the slip after sealing). This is the vocabulary every later chapter silently assumes (the SRS commits, the PCS commits, accumulation commits), so it is installed once, here. (b) **Sigma protocols** are shown as the three-move template every interactive proof in the book is a variation of: the prover *commits*, the verifier sends a random *challenge*, the prover *responds*, and the verifier checks. **Schnorr's identification protocol** — "prove you know the secret exponent behind a public key without revealing it" — is the one fully worked example, kept to the commit-challenge-response beats with a footnote-grade gloss on the cyclic-group arithmetic it rides on. (c) The **simulation paradigm** is given at intuition depth: *if someone with no secret at all could produce a transcript that looks exactly like a real one, then watching the real transcript taught you nothing* — that is what "zero-knowledge" will eventually mean, and it is felt here before it is defined (Ch 9). (d) The **classic demonstrations** make the prover/verifier exchange concrete and physical: **graph 3-coloring** (the prover commits a coloring under cups and reveals only the two cups on a randomly chosen edge — a different coloring each round, so the verifier gains confidence without ever seeing the whole map), **graph isomorphism / non-isomorphism** (the canonical "I can relabel one graph into the other, and prove it by answering a random challenge"), and **quadratic residuosity** (the historical first-ZK-proof artifact). These are the textbook on-ramp from GMR (Ch 1) to Fiat-Shamir (Ch 12); they are the reason the magician/audience picture is *true*, not just evocative. The interlude ends by flagging its own debt: the formal hiding/binding definitions, special soundness, and the knowledge extractor are deferred to Ch 9-10, and the Pedersen construction that actually *builds* the sealed envelope is deferred to Ch 11.
...
- *Classical on-ramp (intuition depth; formal lock-down deferred):* Commitment Scheme (the sealed envelope); hiding; binding; Sigma protocol (commit–challenge–response, the three-move template); Schnorr identification protocol (the worked example); special soundness (named); proof of knowledge (named); simulator / simulation paradigm ("a secret-less simulator could fake the transcript ⇒ nothing leaks"); three-coloring zero-knowledge proof; graph isomorphism / non-isomorphism; quadratic residuosity. These are the GMR-to-Fiat-Shamir historical bridge; the rigor is owed to Ch 9-11.
...
**Reader should already know (prerequisites):**
- *External background* — the everyday meaning of a referee, a coin flip, and a hash (as a "scrambler") — only metaphorically; no cryptographic hash properties yet. Comfort with the idea that a process can use randomness. For the Schnorr worked example only, a *light* touch of **basic modular arithmetic / cyclic groups** ("raising a generator to a power, all mod a prime") — kept to a footnote-grade gloss, not a number-theory prerequisite; the reader who skips the gloss still follows the commit-challenge-response shape.
- *Builds on (internal)* — Chapter 1's three felt promises, the magician metaphor, and the trust-decomposition logline are load-bearing; the chapter is unintelligible without them. Chapter 1's "nothing leaks" promise and named-only simulator become the simulation-paradigm intuition here.
...
- *The sealed envelope (commitment scheme).* Work: makes *hiding* and *binding* tangible at once — you seal a value (it's hidden) and can't swap it later (it's binding), yet you can open it on demand. Where it breaks: a paper envelope can be steamed open or its slip swapped; a cryptographic commitment's hiding and binding are guarantees that rest on a hardness assumption (Pedersen on discrete-log, Ch 11) — don't let "envelope" imply a physical, breakable seal, and don't imply a single envelope can be *perfectly* both hiding and binding (one is computational; the Pedersen construction in Ch 11 makes the trade-off precise).
- *The colored map under cups (graph 3-coloring ZK demo).* Work: dramatizes an interactive proof as a physical game — the verifier points at one edge, the prover lifts exactly two cups showing two different colors, and a fresh random recoloring each round means the verifier is convinced the map is colorable without ever seeing the coloring. Where it breaks: the security is *per-round and probabilistic* — one round barely convinces; confidence accrues over many rounds (the soundness math is Ch 7-9), and the "fresh recoloring each round" is doing the zero-knowledge work the metaphor should not gloss over (reuse the same coloring and you leak).
...
- **The formal lock-down of the classical on-ramp** (commitment *hiding/binding* defined rigorously, *special soundness*, the *knowledge extractor*) → Chapters 9–10 (the Sigma/Schnorr pattern met here, formalized there).
- **The Pedersen commitment construction** that actually builds the sealed envelope → Chapter 11 (named here as a forward debt; IPA in Ch 11 is a Pedersen-vector argument).
- Quadratic-residuosity / graph-isomorphism *internals* beyond the demonstration sketch → out of scope; they ride along as the canonical interactive-ZK examples, not as built protocols.
...
- *Aggregation / production:* **Mina (Pickles) — the production-IVC exemplar: a recursively-composed constant-size blockchain (C18)** · Proof Aggregation (preview) · Recursive STARK proving · Recursive SNARK · Universal Proof Aggregation (UPA) · SHARP / SHARed Prover
- *Conceptual bridges:* bootstrapping ↔ IVC parallel · Halo / nested amortization (recursion without a trusted setup) · succinct blockchain / high-integrity certification framing (C18, instantiated by Mina)
...
- *Web & interop:* zkTLS / DECO; zkBridge; ZK light clients; cross-chain bridge proofs.
...
- *Primitives & market:* Verifiable Delay Function (VDF); Wesolowski / Pietrzak constructions; the prover market; **deVirgo / distributed (collaborative) proof generation; proving-as-a-service / prover networks**; maturity tiers (production / growth / pilot / research).
