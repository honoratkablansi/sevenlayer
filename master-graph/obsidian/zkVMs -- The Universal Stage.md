---
source_file: "wiki/chapters/11-zkvms-the-universal-stage.md"
type: "document"
community: "Community 6"
tags:
  - graphify/document
  - graphify/EXTRACTED
  - community/Community_6
---

# zkVMs -- The Universal Stage

## Connections
- [[AIR (Algebraic Intermediate Representation)]] - `references` [EXTRACTED]
- [[Airbender]] - `references` [EXTRACTED]
- [[BLS12-381 Curve]] - `references` [EXTRACTED]
- [[BN254 (alt_bn128) Curve]] - `references` [EXTRACTED]
- [[BabyBear]] - `references` [EXTRACTED]
- [[Cairo (StarkWare ZK-native ISA)]] - `references` [EXTRACTED]
- [[Circle STARKs]] - `references` [EXTRACTED]
- [[Evaluating Compiler Optimization Impacts on zkVM Performance]] - `cites` [EXTRACTED]
- [[FRI Commitment Scheme]] - `references` [EXTRACTED]
- [[Folding Scheme]] - `references` [EXTRACTED]
- [[Goldilocks Field_1]] - `references` [EXTRACTED]
- [[Groth16]] - `references` [EXTRACTED]
- [[Hyrax Commitment Scheme]] - `references` [EXTRACTED]
- [[Jolt (zkVM via Lookups)]] - `references` [EXTRACTED]
- [[KZG Polynomial Commitments from pairings (with trusted setup); Dory transparent pairing-based scheme]] - `references` [EXTRACTED]
- [[Lasso]] - `references` [EXTRACTED]
- [[Lattice Cryptography]] - `references` [EXTRACTED]
- [[LogUp]] - `references` [EXTRACTED]
- [[Lookup Argument]] - `references` [EXTRACTED]
- [[Mersenne-31  M31]] - `references` [EXTRACTED]
- [[Mersenne-31 Field]] - `references` [EXTRACTED]
- [[Midnight (Privacy Blockchain)]] - `references` [EXTRACTED]
- [[Neo Lattice-based Folding Scheme for CCS over Small Fields and Pay-per-bit Commitments]] - `cites` [EXTRACTED]
- [[Nova]] - `references` [EXTRACTED]
- [[PLONK]] - `references` [EXTRACTED]
- [[Performance The Cost Collapse]] - `part_of` [EXTRACTED]
- [[Poseidon Hash_1]] - `references` [EXTRACTED]
- [[Proving Nothing]] - `part_of` [EXTRACTED]
- [[RISC Zero]] - `references` [EXTRACTED]
- [[RISC-V Convergence]] - `part_of` [EXTRACTED]
- [[RISC-V architecture]] - `references` [EXTRACTED]
- [[SP1 Hypercube (Succinct)]] - `references` [EXTRACTED]
- [[SP1 Hypercube Proving Ethereum in Real-Time (Succinct Labs)]] - `references` [EXTRACTED]
- [[STARK]] - `references` [EXTRACTED]
- [[Sevenlayer Wiki Index]] - `references` [EXTRACTED]
- [[Shipping an L1 zkEVM 2 The Security Foundations (Ethereum Foundation)]] - `references` [EXTRACTED]
- [[Small prime fields (machine-register-sized Goldilocks, M61)]] - `references` [EXTRACTED]
- [[Spartan]] - `references` [EXTRACTED]
- [[Stwo]] - `references` [EXTRACTED]
- [[Sum-Check Protocol_1]] - `references` [EXTRACTED]
- [[The Proof Core Triad]] - `part_of` [EXTRACTED]
- [[The Landscape Table (March 2026)]] - `part_of` [EXTRACTED]
- [[The Stage Is Set]] - `part_of` [EXTRACTED]
- [[The Witness Gap_1]] - `defines` [EXTRACTED]
- [[Three zkVMs Through Seven Layers]] - `part_of` [EXTRACTED]
- [[Trusted Setup Ceremony]] - `references` [EXTRACTED]
- [[ZKsync Airbender GPU-Accelerated RISC-V Proving]] - `references` [EXTRACTED]
- [[zkVM]] - `references` [EXTRACTED]
- [[zkVMs Across the Stack]] - `part_of` [EXTRACTED]

#graphify/document #graphify/EXTRACTED #community/Community_6