---
source_file: "references/recursion/ch1/ref-09-bcms-pcd-accumulation.pdf"
type: "paper"
community: "Community 60"
location: "§2.1 (A new paradigm), §5.2"
tags:
  - graphify/paper
  - graphify/EXTRACTED
  - community/Community_60
---

# Relaxed Succinctness: deferred verification via sublinear accumulation verifier

## Connections
- [[Accumulation Schemes]] - `conceptually_related_to` [EXTRACTED]
- [[Proof-Carrying Data from Accumulation Schemes (BCMS20)]] - `introduces` [EXTRACTED]
- [[SNARK (Succinct Non-interactive ARgument of Knowledge)]] - `conceptually_related_to` [EXTRACTED]
- [[Theorem 5.2 transformation T from SNARK + accumulation scheme to PCD for constant-depth predicates]] - `assumes` [EXTRACTED]

#graphify/paper #graphify/EXTRACTED #community/Community_60