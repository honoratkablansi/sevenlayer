---
source_file: "wiki/chapters/07-the-deep-craft.md"
type: "document"
community: "Community 10"
tags:
  - graphify/document
  - graphify/EXTRACTED
  - community/Community_10
---

# Lattice Cryptography

## Connections
- [[Ajtai commitments]] - `conceptually_related_to` [INFERRED]
- [[CCS The Rosetta Stone]] - `references` [EXTRACTED]
- [[Case Study Midnight's Sealed Certificate]] - `references` [EXTRACTED]
- [[Coda]] - `references` [EXTRACTED]
- [[Convergence]] - `references` [EXTRACTED]
- [[Encoding the Performance]] - `references` [EXTRACTED]
- [[FIPS 203 Module-Lattice-Based Key-Encapsulation Mechanism Standard (with FIPS 204205)]] - `conceptually_related_to` [EXTRACTED]
- [[FIPS 204 Module-Lattice-Based Digital Signature Standard]] - `references` [EXTRACTED]
- [[Folding Scheme]] - `used_in` [EXTRACTED]
- [[Four Families of Commitment Schemes]] - `references` [EXTRACTED]
- [[Fully Homomorphic Encryption Using Ideal Lattices]] - `conceptually_related_to` [EXTRACTED]
- [[Functional Commitment]] - `used_to_build` [EXTRACTED]
- [[Greyhound polynomial commitment scheme]] - `conceptually_related_to` [EXTRACTED]
- [[Greyhound Fast Polynomial Commitments from Lattices]] - `references` [EXTRACTED]
- [[How to Read This Guide]] - `conceptually_related_to` [EXTRACTED]
- [[Ideal Lattice]] - `relates_to` [EXTRACTED]
- [[Ideal lattices (ideals of Zxf(x) as lattices; rotation basis)]] - `conceptually_related_to` [EXTRACTED]
- [[LaBRADOR (Lattice Argument System)]] - `references` [EXTRACTED]
- [[LaBRADOR Proof System]] - `uses` [EXTRACTED]
- [[Lattice-Based Functional Commitments Fast Verification and Cryptanalysis]] - `references` [EXTRACTED]
- [[Lattice-Based Proving]] - `references` [EXTRACTED]
- [[LatticeFold  LatticeFold+]] - `conceptually_related_to` [EXTRACTED]
- [[LatticeFold+ BC25 (monomial embedding & exact range proof, cited prior work)]] - `conceptually_related_to` [INFERRED]
- [[LatticeFold+ Faster, Simpler, Shorter Lattice-Based Folding for Succinct Proof Systems]] - `conceptually_related_to` [EXTRACTED]
- [[LatticeFold A Lattice-based Folding Scheme and its Applications to Succinct Proof Systems]] - `conceptually_related_to` [EXTRACTED]
- [[Layer 1 -- Building the Stage]] - `references` [EXTRACTED]
- [[Layer 5 -- The Sealed Certificate]] - `references` [EXTRACTED]
- [[Layer 6 -- The Deep Craft]] - `references` [EXTRACTED]
- [[ML-DSA (Module-Lattice Digital Signature Algorithm)]] - `based_on` [EXTRACTED]
- [[Midnight -- The Privacy Theater]] - `references` [EXTRACTED]
- [[Midnight's ZKIR A Concrete Layer 4]] - `references` [EXTRACTED]
- [[Module Lattice]] - `instance_of` [EXTRACTED]
- [[Module-Ajtai binding commitment (Construction 2.1) with strictrelaxed openings]] - `conceptually_related_to` [EXTRACTED]
- [[Module-SIS  Module-LWE]] - `uses` [EXTRACTED]
- [[Multiplicative Relation Proof]] - `uses` [EXTRACTED]
- [[NIST PQC Standards (FIPS 203204205, IR 8547)]] - `references` [EXTRACTED]
- [[Neo NS25 (lattice SNARK; Hadamard sumcheck and single-instance encoding, cited prior work)]] - `conceptually_related_to` [INFERRED]
- [[Neo Lattice-based Folding Scheme for CCS over Small Fields and Pay-per-bit Commitments]] - `conceptually_related_to` [EXTRACTED]
- [[Nightstream Lattice-Based Folding for CCS (LFDT-Nightstream)]] - `references` [EXTRACTED]
- [[Nightstream What a Folding Engine Looks Like from the Inside]] - `references` [EXTRACTED]
- [[Open Questions and the Road Ahead]] - `references` [EXTRACTED]
- [[Path Three Post-Quantum Folding]] - `conceptually_related_to` [EXTRACTED]
- [[Performance The Cost Collapse]] - `conceptually_related_to` [EXTRACTED]
- [[Polynomial Commitment Scheme]] - `uses` [EXTRACTED]
- [[Post-Quantum Cryptography]] - `uses` [EXTRACTED]
- [[Post-quantum security (against quantum adversaries; security in the QROM)]] - `uses` [EXTRACTED]
- [[Power-of-Two Cyclotomic Ring ZX(Xd+1)]] - `relates_to` [EXTRACTED]
- [[Privacy-Enhancing Technologies]] - `references` [EXTRACTED]
- [[Quantum Shelf Life & Option Value]] - `references` [EXTRACTED]
- [[SNARK (Succinct Non-interactive ARgument of Knowledge)]] - `uses` [EXTRACTED]
- [[Short Integer Solutions (SIS) Assumption]] - `part_of` [EXTRACTED]
- [[Sigma protocol (public-coin 3-move)]] - `uses` [EXTRACTED]
- [[Succinct Argument]] - `uses` [EXTRACTED]
- [[Symphony Scalable SNARKs in the Random Oracle Model from Lattice-Based High-Arity Folding]] - `references` [EXTRACTED]
- [[The Binary That Broke]] - `conceptually_related_to` [EXTRACTED]
- [[The Deepest Question]] - `conceptually_related_to` [EXTRACTED]
- [[The Folding Genealogy]] - `references` [EXTRACTED]
- [[The Four Pillars]] - `conceptually_related_to` [EXTRACTED]
- [[The Landscape Table (March 2026)]] - `conceptually_related_to` [EXTRACTED]
- [[The Laws That Break]] - `references` [EXTRACTED]
- [[The One-Way Door]] - `references` [EXTRACTED]
- [[The Post-Quantum Horizon]] - `references` [EXTRACTED]
- [[The Promise of Provable and Programmable Secrets]] - `references` [EXTRACTED]
- [[The Proof Core Why Layers 4, 5, and 6 Are Inseparable]] - `references` [EXTRACTED]
- [[The Quantum Shelf Life]] - `conceptually_related_to` [EXTRACTED]
- [[The Quantum Threat Horizon]] - `references` [EXTRACTED]
- [[The Sealed Certificate (Chapter Closing)]] - `references` [EXTRACTED]
- [[The Seven Layers at a Glance]] - `conceptually_related_to` [EXTRACTED]
- [[The Seven Questions That Remain Open]] - `references` [EXTRACTED]
- [[The Structural Advantage of Lattices]] - `references` [EXTRACTED]
- [[The Synthesis -- Three Paths, Not Two]] - `references` [EXTRACTED]
- [[The Three-Path Table]] - `conceptually_related_to` [EXTRACTED]
- [[The Trilemma -- And Its Dissolution]] - `references` [EXTRACTED]
- [[Three Hardness Assumptions, Three Worlds]] - `references` [EXTRACTED]
- [[Trust Decomposition Seven Weaker Assumptions]] - `conceptually_related_to` [EXTRACTED]
- [[Where Midnight Challenges the Model]] - `references` [EXTRACTED]
- [[Where the Layers Collapse]] - `references` [EXTRACTED]
- [[Zero-Knowledge Proof]] - `uses` [EXTRACTED]
- [[zkVMs -- The Universal Stage]] - `references` [EXTRACTED]

#graphify/document #graphify/EXTRACTED #community/Community_10