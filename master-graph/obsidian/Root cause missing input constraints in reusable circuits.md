---
source_file: "references/ch01/ref-03-sok-snark-vulnerabilities.pdf"
type: "paper"
community: "Community 2"
location: "§5.2 R2, App B.1.2"
tags:
  - graphify/paper
  - graphify/EXTRACTED
  - community/Community_2
---

# Root cause: missing input constraints in reusable circuits

## Connections
- [[Scroll zkEVM LtChip missing range-check bug (Halo2 missing input constraint)]] - `conceptually_related_to` [EXTRACTED]
- [[Under-Constrained Circuit  Missing Constraint]] - `conceptually_related_to` [EXTRACTED]

#graphify/paper #graphify/EXTRACTED #community/Community_2