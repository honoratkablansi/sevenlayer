---
source_file: "proving-nothing.md"
type: "document"
community: "Community 17"
location: "Chapter 3"
tags:
  - graphify/document
  - graphify/EXTRACTED
  - community/Community_17
---

# ZKIR (Zero-Knowledge Intermediate Representation)

## Connections
- [[Case Study Midnight (Layer 6)]] - `references` [EXTRACTED]
- [[Case Study Midnight's Sealed Certificate]] - `references` [EXTRACTED]
- [[Choreographing the Act]] - `references` [EXTRACTED]
- [[Compact (MidnightIOG DSL)]] - `defines` [EXTRACTED]
- [[Compact Language Reference (Midnight Network)]] - `references` [EXTRACTED]
- [[Encoding the Performance]] - `references` [EXTRACTED]
- [[Five Lessons for ZK System Design]] - `references` [EXTRACTED]
- [[Full Seven-Layer Mapping]] - `references` [EXTRACTED]
- [[Midnight (Privacy Blockchain)]] - `part_of` [EXTRACTED]
- [[Midnight -- The Privacy Theater]] - `defines` [EXTRACTED]
- [[Midnight Reference Documentation (473pp)]] - `references` [EXTRACTED]
- [[Midnight at a Glance]] - `references` [EXTRACTED]
- [[Midnight's ZKIR A Concrete Layer 4]] - `defines` [EXTRACTED]
- [[Midnight Compiler, IR, Circuit]] - `defines` [EXTRACTED]
- [[PLONKish Arithmetization]] - `conceptually_related_to` [EXTRACTED]
- [[Poseidon Hash]] - `references` [EXTRACTED]
- [[The Disclose Boundary Midnight's Witness Architecture]] - `references` [EXTRACTED]
- [[The Secret Performance]] - `references` [EXTRACTED]
- [[Where Midnight Challenges the Model]] - `references` [EXTRACTED]
- [[Where Midnight Validates the Model]] - `references` [EXTRACTED]
- [[Where the Analogies Break]] - `references` [EXTRACTED]
- [[ZKIR Intermediate Representation Reference (Midnight Network, stub)]] - `references` [EXTRACTED]

#graphify/document #graphify/EXTRACTED #community/Community_17