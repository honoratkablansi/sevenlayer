---
source_file: "references/ch14/ref-45-small-space-cpu-proofs.pdf"
type: "document"
community: "Community 88"
location: "page 3"
tags:
  - graphify/document
  - graphify/EXTRACTED
  - community/Community_88
---

# Twist and Shout Memory-Checking Arguments

## Connections
- [[Argument that large shards enable faster TwistShout proving (K-linear cost amortized)]] - `conceptually_related_to` [EXTRACTED]
- [[Concrete cost estimate ~900T linear-space vs ~900T+12T log T small-space field mults]] - `conceptually_related_to` [EXTRACTED]
- [[Dimension parameter d factoring one-hot address poly into d smaller polys]] - `defines` [EXTRACTED]
- [[Jolt sum-check-based zkVM for RISC-V]] - `assumes` [EXTRACTED]
- [[One-hot (unit-vector) address encoding for memory ops in TwistShout]] - `conceptually_related_to` [EXTRACTED]
- [[Proving CPU Executions in Small Space]] - `references` [EXTRACTED]
- [[Shout sum-check memory-checking  lookup argument for read-only memory]] - `conceptually_related_to` [EXTRACTED]
- [[Twist sum-check memory-checking argument for readwrite memory]] - `conceptually_related_to` [EXTRACTED]

#graphify/document #graphify/EXTRACTED #community/Community_88