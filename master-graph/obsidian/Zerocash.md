---
source_file: "proving-nothing.md"
type: "document"
community: "Community 25"
location: "Chapter 9"
tags:
  - graphify/document
  - graphify/EXTRACTED
  - community/Community_25
---

# Zerocash

## Connections
- [[Anonymity set]] - `conceptually_related_to` [EXTRACTED]
- [[Blockchain Privacy and Regulatory Compliance Towards a Practical Equilibrium]] - `cites` [EXTRACTED]
- [[Decentralized Anonymous Payment]] - `defines` [EXTRACTED]
- [[Decentralized Private Computation (DPC) scheme]] - `assumes` [EXTRACTED]
- [[Ideal private payments contract (Delta_pp, Lambda_pp)]] - `conceptually_related_to` [EXTRACTED]
- [[Kachina]] - `references` [EXTRACTED]
- [[Kachina -- Foundations of Private Smart Contracts]] - `cites` [EXTRACTED]
- [[Provisions privacy-preserving proof of solvency for Bitcoin exchanges]] - `cites` [EXTRACTED]
- [[Record (commitment, address pk, payload, birthdeath predicate, serial number)]] - `conceptually_related_to` [EXTRACTED]
- [[SNARK (Succinct Non-interactive ARgument of Knowledge)]] - `uses` [EXTRACTED]
- [[Zero-Knowledge Proof]] - `uses` [EXTRACTED]
- [[Zerocash Kachina contract Gamma_zc]] - `introduces` [EXTRACTED]
- [[Zexe a ledger-based system for decentralized private computation]] - `cites` [EXTRACTED]
- [[zk-SNARK]] - `assumes` [EXTRACTED]

#graphify/document #graphify/EXTRACTED #community/Community_25