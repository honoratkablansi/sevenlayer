---
source_file: "wiki/chapters/03-choreographing-the-act.md"
type: "document"
community: "Community 13"
tags:
  - graphify/document
  - graphify/EXTRACTED
  - community/Community_13
---

# Choreographing the Act

## Connections
- [[ACIR (Abstract Circuit Intermediate Representation)]] - `references` [EXTRACTED]
- [[Automated Detection of Under-Constrained Circuits in Zero-Knowledge Proofs]] - `cites` [EXTRACTED]
- [[BLS12-381 Curve]] - `references` [EXTRACTED]
- [[BN254 (alt_bn128) Curve]] - `references` [EXTRACTED]
- [[Cairo (StarkWare ZK-native ISA)]] - `references` [EXTRACTED]
- [[CirC Compiler Infrastructure for Proof Systems, Software Verification, and More]] - `cites` [EXTRACTED]
- [[Circom]] - `references` [EXTRACTED]
- [[Compact Language]] - `references` [EXTRACTED]
- [[Compact's Disclosure Analysis]] - `part_of` [EXTRACTED]
- [[Disclosure Analysis (Compact)]] - `defines` [EXTRACTED]
- [[Evaluating Compiler Optimization Impacts on zkVM Performance]] - `cites` [EXTRACTED]
- [[FRI Commitment Scheme]] - `references` [EXTRACTED]
- [[Folding Scheme]] - `references` [EXTRACTED]
- [[From Circuits to Virtual Machines A Brief Evolution]] - `part_of` [EXTRACTED]
- [[Goldilocks Field]] - `references` [EXTRACTED]
- [[Groth16]] - `references` [EXTRACTED]
- [[Halo 2  UltraPlonk]] - `references` [EXTRACTED]
- [[Jolt (zkVM via Lookups)]] - `references` [EXTRACTED]
- [[Leo (Aleo)]] - `defines` [EXTRACTED]
- [[Midnight (Privacy Blockchain)]] - `references` [EXTRACTED]
- [[Midnight Compiler, IR, Circuit]] - `part_of` [EXTRACTED]
- [[Noir (Aztec)]] - `defines` [EXTRACTED]
- [[PLONK]] - `references` [EXTRACTED]
- [[Plonky3]] - `references` [EXTRACTED]
- [[Poseidon Hash]] - `references` [EXTRACTED]
- [[Practical Security Analysis of Zero-Knowledge Proof Circuits]] - `cites` [EXTRACTED]
- [[Proving Nothing]] - `part_of` [EXTRACTED]
- [[R1CS (Rank-1 Constraint Systems)]] - `references` [EXTRACTED]
- [[RISC Zero]] - `references` [EXTRACTED]
- [[RISC-V Won. Why Taxonomy Still Matters.]] - `part_of` [EXTRACTED]
- [[RISC-V architecture]] - `references` [EXTRACTED]
- [[SP1 Hypercube (Succinct)]] - `references` [EXTRACTED]
- [[Sevenlayer Wiki Index]] - `references` [EXTRACTED]
- [[SoK What Don't We Know Understanding Security Vulnerabilities in SNARKs]] - `cites` [EXTRACTED]
- [[Stwo]] - `references` [EXTRACTED]
- [[Sudoku running example]] - `references` [EXTRACTED]
- [[The Developer's Actual Experience]] - `part_of` [EXTRACTED]
- [[The Four Philosophies]] - `part_of` [EXTRACTED]
- [[Tornado Cash]] - `references` [EXTRACTED]
- [[Under-Constrained Circuit  Missing Constraint]] - `references` [EXTRACTED]
- [[Under-Constrained Circuits The Dominant Failure Mode]] - `part_of` [EXTRACTED]
- [[ZKIR (Zero-Knowledge Intermediate Representation)]] - `references` [EXTRACTED]
- [[zkEVM  EVM-Compatible Proving]] - `references` [EXTRACTED]
- [[zkVM]] - `references` [EXTRACTED]

#graphify/document #graphify/EXTRACTED #community/Community_13