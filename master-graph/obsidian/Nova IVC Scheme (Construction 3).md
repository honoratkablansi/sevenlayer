---
source_file: "references/ch06/ref-17-nova.pdf"
type: "paper"
community: "Community 33"
location: "§5.1 Construction 3"
tags:
  - graphify/paper
  - graphify/EXTRACTED
  - community/Community_33
---

# Nova IVC Scheme (Construction 3)

## Connections
- [[Augmented Function F' (verifier circuit folding step)]] - `defines` [EXTRACTED]
- [[Constant Recursion Overhead (~10,000 R1CS constraints, two scalar mults)]] - `conceptually_related_to` [EXTRACTED]
- [[Incrementally Verifiable Computation (IVC)]] - `defines` [EXTRACTED]
- [[Non-Interactive Folding Scheme via Fiat-Shamir (Construction 2)]] - `assumes` [EXTRACTED]
- [[Nova IVC Knowledge Soundness (Lemma 3  Lemma 10)]] - `proves` [EXTRACTED]
- [[Nova Recursive Zero-Knowledge Arguments from Folding Schemes]] - `introduces` [EXTRACTED]
- [[zkSNARK of a Valid IVC Proof (Construction 4)]] - `assumes` [EXTRACTED]

#graphify/paper #graphify/EXTRACTED #community/Community_33