---
source_file: "references/ch07/ref-24-shor.pdf"
type: "paper"
community: "Community 56"
location: "p.1 (abstract)"
tags:
  - graphify/paper
  - graphify/EXTRACTED
  - community/Community_56
---

# Discrete Logarithm Problem

## Connections
- [[Algorithms for Quantum Computation Discrete Logarithms and Factoring]] - `conceptually_related_to` [EXTRACTED]
- [[Embedding degree k]] - `relates_to` [EXTRACTED]
- [[Extended Tower Number Field Sieve A New Complexity for the Medium Prime Case]] - `references` [EXTRACTED]
- [[Finite Field Arithmetic]] - `conceptually_related_to` [EXTRACTED]
- [[Finite Fields]] - `defined_in` [EXTRACTED]
- [[Layer 1 -- Building the Stage]] - `defines` [EXTRACTED]
- [[Number Field Sieve]] - `used_for` [EXTRACTED]
- [[Quantum discrete logarithm algorithm]] - `defines` [EXTRACTED]
- [[Quantum-vulnerable key establishment (Finite FieldEC DH & MQV per SP 800-56A, RSA per SP 800-56B)]] - `assumes` [EXTRACTED]
- [[exTNFS (Extended Tower Number Field Sieve)]] - `introduces` [EXTRACTED]

#graphify/paper #graphify/EXTRACTED #community/Community_56