---
source_file: "proving-nothing.md"
type: "document"
community: "Community 95"
location: "Chapter 3"
tags:
  - graphify/document
  - graphify/EXTRACTED
  - community/Community_95
---

# Circom

## Connections
- [[Arithmetic Circuit]] - `defines` [EXTRACTED]
- [[Automated Detection of Under-Constrained Circuits in Zero-Knowledge Proofs]] - `conceptually_related_to` [EXTRACTED]
- [[BigLessThan motivating example (Circom bug at line 32)]] - `cites` [EXTRACTED]
- [[Certifying Zero-Knowledge Circuits with Refinement Types]] - `references` [EXTRACTED]
- [[Choreographing the Act]] - `references` [EXTRACTED]
- [[Circomlib Decoder Underconstrained Bug (motivating example)]] - `conceptually_related_to` [EXTRACTED]
- [[Circomspect Static Analyzer]] - `conceptually_related_to` [EXTRACTED]
- [[Coda statically-typed functional DSL for ZK circuits]] - `conceptually_related_to` [EXTRACTED]
- [[Evaluation 77 circuits from 9 Circom projects, 66 verified]] - `cites` [EXTRACTED]
- [[Five Lessons for ZK System Design]] - `references` [EXTRACTED]
- [[From Circuits to Virtual Machines A Brief Evolution]] - `references` [EXTRACTED]
- [[Jordi Baylina (Circom, iden3, Hermez, ZisK)]] - `references` [EXTRACTED]
- [[Key finding low-level DSLs (Circom, Gnark) are easy-to-misuse, driving most circuit bugs]] - `cites` [EXTRACTED]
- [[Midnight Compiler, IR, Circuit]] - `references` [EXTRACTED]
- [[Practical Security Analysis of Zero-Knowledge Proof Circuits]] - `assumes` [EXTRACTED]
- [[R1CS (Rank-1 Constraint Systems)]] - `defines` [EXTRACTED]
- [[Root cause assigned but not constrained (signal given a value with no constraint)]] - `conceptually_related_to` [EXTRACTED]
- [[SNARK (Succinct Non-interactive ARgument of Knowledge)]] - `used` [EXTRACTED]
- [[The Developer's Actual Experience]] - `references` [EXTRACTED]
- [[Tornado Cash Under-Constraint Bug]] - `references` [EXTRACTED]
- [[Tornado Cash circomlib MiMC under-constrained bug (assigned-but-unconstrained output)]] - `conceptually_related_to` [EXTRACTED]
- [[Trust Decomposition Seven Weaker Assumptions]] - `references` [EXTRACTED]
- [[Under-Constrained Circuits]] - `conceptually_related_to` [EXTRACTED]
- [[Under-Constrained Circuits The Dominant Failure Mode]] - `references` [EXTRACTED]
- [[Witness-Constraint Divergence]] - `references` [EXTRACTED]
- [[Xue et al., ZK-Coder LLMs for ZK Proof Code Generation, arXiv 2509.11708]] - `references` [EXTRACTED]
- [[ZkBench Benchmark Suite (163 Circom circuits)]] - `conceptually_related_to` [EXTRACTED]
- [[zkSNARK]] - `conceptually_related_to` [EXTRACTED]

#graphify/document #graphify/EXTRACTED #community/Community_95