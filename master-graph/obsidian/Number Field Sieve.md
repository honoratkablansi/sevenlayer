---
source_file: "references/ch10/ref-50-extended-tower-nfs.pdf"
type: "document"
community: "Community 16"
location: "page 1"
tags:
  - graphify/document
  - graphify/EXTRACTED
  - community/Community_16
---

# Number Field Sieve

## Connections
- [[Bilinear Pairing]] - `attacks` [EXTRACTED]
- [[Comparing the Pairing Efficiency over Composite-Order and Prime-Order Elliptic Curves]] - `references` [EXTRACTED]
- [[Composite-Order Bilinear Groups]] - `attacks` [EXTRACTED]
- [[Discrete Logarithm Problem]] - `used_for` [EXTRACTED]
- [[Extended Tower Number Field Sieve A New Complexity for the Medium Prime Case]] - `references` [EXTRACTED]
- [[NFSECM crossover ECM beats NFS at 5+ prime factors]] - `assumes` [EXTRACTED]
- [[Polynomial Selection]] - `part_of` [EXTRACTED]
- [[Tower Number Field Sieve (TNFS)]] - `extends` [EXTRACTED]
- [[Tower Number Field Sieve (TNFS) Barbulescu-Gaudry-Kleinjung]] - `conceptually_related_to` [EXTRACTED]
- [[exTNFS (Extended Tower Number Field Sieve)]] - `conceptually_related_to` [EXTRACTED]

#graphify/document #graphify/EXTRACTED #community/Community_16