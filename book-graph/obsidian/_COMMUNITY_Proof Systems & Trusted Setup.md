---
type: community
cohesion: 0.07
members: 45
---

# Proof Systems & Trusted Setup

**Cohesion:** 0.07 - loosely connected
**Members:** 45 nodes

## Members
- [[1-of-N Trust Model]] - document - proving-nothing.md
- [[ADOPT Framework (Available, Decentralized, Open, Persistent, Transparent)]] - document - proving-nothing.md
- [[BLS12-381 Curve]] - document - proving-nothing.md
- [[Ben-Sasson et al. — Scalable, Transparent, Post-Quantum Secure Computational Integrity (ePrint 2018046)]] - document - proving-nothing.md
- [[Bulletproofs  Inner Product Argument]] - document - proving-nothing.md
- [[CVE-2019-7167 BCTV14 Counterfeiting Bug]] - document - proving-nothing.md
- [[CapexOpex Setup Economics Framework]] - document - proving-nothing.md
- [[Chaliasos et al. — SoK Understanding Security Vulnerabilities in SNARKs (USENIX Security 2024)]] - document - proving-nothing.md
- [[Chapter 2 Layer 1 — Building the Stage]] - document - proving-nothing.md
- [[Ethereum Foundation L1 zkEVM Mandate]] - document - proving-nothing.md
- [[Ethereum KZG Summoning Ceremony (2023, 141,416 contributors)]] - document - proving-nothing.md
- [[Ethereum KZG Summoning of 2023 (141,416 participants)]] - document - proving-nothing.md
- [[FFLONK]] - document - proving-nothing.md
- [[FRI Protocol]] - document - proving-nothing.md
- [[Gabizon, Williamson, Ciobotaru — PLONK (ePrint 2019953)]] - document - proving-nothing.md
- [[Gennaro, Gentry, Parno, Raykova — QAP (Eurocrypt 2013)]] - document - proving-nothing.md
- [[Groth 2016 (ePrint 2016260)]] - document - proving-nothing.md
- [[Groth — On the Size of Pairing-Based Non-interactive Arguments (EUROCRYPT 2016)]] - document - proving-nothing.md
- [[Groth16]] - document - proving-nothing.md
- [[Hybrid Transparent-Inner  Compact-Outer Architecture]] - document - proving-nothing.md
- [[Jubjub Curve]] - document - proving-nothing.md
- [[Layer 5 The Sealed Certificate (Proof System)]] - document - proving-nothing.md
- [[Luhmann — Risk A Sociological Theory (trust vs confidence, 1993)]] - document - proving-nothing.md
- [[Maller et al. — Sonic Universal and Updatable SRS (CCS 2019)]] - document - proving-nothing.md
- [[Midnight BLS12-381 Layer 1 Case Study (CompactZKIR, Pluto-Eris switch)]] - document - proving-nothing.md
- [[Nikolaenko et al. — Powers-of-Tau to the People Decentralizing Setup Ceremonies (ACNS 2024)]] - document - proving-nothing.md
- [[PLONK]] - document - proving-nothing.md
- [[Powers of Tau]] - document - proving-nothing.md
- [[Quadratic Arithmetic Programs (QAP)]] - document - proving-nothing.md
- [[Real-Time Ethereum Proving]] - document - proving-nothing.md
- [[SNARK (Succinct Non-interactive ARgument of Knowledge)]] - document - proving-nothing.md
- [[SP1 Hypercube (Succinct Labs)]] - document - proving-nothing.md
- [[STARK]] - document - proving-nothing.md
- [[STARK-Inside-SNARK-Outside Hybrid Pipeline]] - document - proving-nothing.md
- [[Schwartz-Zippel Lemma_1]] - document - proving-nothing.md
- [[The Proof Core Triad (Field, Commitment, Arithmetization)]] - document - proving-nothing.md
- [[The SNARK-or-STARK Binary Is Dead]] - document - proving-nothing.md
- [[Toxic Waste (Trapdoor)]] - document - proving-nothing.md
- [[Transparent Setup]] - document - proving-nothing.md
- [[Trusted Setup Ceremony]] - document - proving-nothing.md
- [[Universal vs Circuit-Specific SRS]] - document - proving-nothing.md
- [[Wang, Cohney, Bonneau — SoK Trusted Setups for Powers-of-Tau Strings (FC 2025)]] - document - proving-nothing.md
- [[ZKsync Airbender]] - document - proving-nothing.md
- [[Zcash Sapling Ceremony (2018, BGM17 MMORPG)]] - document - proving-nothing.md
- [[Zcash Sprout Ceremony (2016, BCTV14)]] - document - proving-nothing.md

## Live Query (requires Dataview plugin)

```dataview
TABLE source_file, type FROM #community/Proof_Systems__Trusted_Setup
SORT file.name ASC
```

## Connections to other communities
- 11 edges to [[_COMMUNITY_Commitments & Cryptographic Hardness]]
- 5 edges to [[_COMMUNITY_Fiat-Shamir Security & Synthesis]]
- 5 edges to [[_COMMUNITY_Circuit Security & Constraint Systems]]
- 4 edges to [[_COMMUNITY_Lookups & zkVM Arithmetic]]
- 4 edges to [[_COMMUNITY_Verification & Rollup Economics]]
- 4 edges to [[_COMMUNITY_Arithmetization & ZK Languages]]
- 3 edges to [[_COMMUNITY_Midnight & Private Contracts]]
- 1 edge to [[_COMMUNITY_ZK Foundations & Framing]]
- 1 edge to [[_COMMUNITY_Compilation & Witness Generation]]

## Top bridge nodes
- [[Groth16]] - degree 25, connects to 6 communities
- [[BLS12-381 Curve]] - degree 12, connects to 5 communities
- [[STARK]] - degree 16, connects to 4 communities
- [[Trusted Setup Ceremony]] - degree 15, connects to 4 communities
- [[Chapter 2 Layer 1 — Building the Stage]] - degree 4, connects to 2 communities