---
type: community
cohesion: 0.06
members: 38
---

# Verification & Rollup Economics

**Cohesion:** 0.06 - loosely connected
**Members:** 38 nodes

## Members
- [[Aligned Layer]] - document - proving-nothing.md
- [[Beanstalk Flash Loan Attack ($182M, 2022)]] - document - proving-nothing.md
- [[Chaliasos et al. Pricing Attacks (2025)]] - document - proving-nothing.md
- [[Claim Proving Cost Collapse $80 to $0.04 (2023-2025)]] - document - proving-nothing.md
- [[DA Marketplace (Celestia, EigenDA, Avail)]] - document - proving-nothing.md
- [[DA-Saturation Attack]] - document - proving-nothing.md
- [[Data Availability]] - document - proving-nothing.md
- [[EIP-4844 (Blob Transactions)]] - document - proving-nothing.md
- [[FOOM Club Verifier Misconfiguration (2022)]] - document - proving-nothing.md
- [[Governance Attack Surface]] - document - proving-nothing.md
- [[L2Beat Stages Framework]] - document - proving-nothing.md
- [[Layer 7 -- The Verdict]] - document - proving-nothing.md
- [[LazyLedger (Al-Bassam, 2019)]] - document - proving-nothing.md
- [[Linea (ConsenSys zkEVM, $2B TVL, gnark)]] - document - proving-nothing.md
- [[Meisami & Bodell Upgradeable Contract Survey (2023)]] - document - proving-nothing.md
- [[Multidimensional Fee Pricing]] - document - proving-nothing.md
- [[NEBRA]] - document - proving-nothing.md
- [[On-Chain Verifier]] - document - proving-nothing.md
- [[Polygon zkEVM Shutdown (~$250M, ZisK spinoff)]] - document - proving-nothing.md
- [[Proof Aggregation]] - document - proving-nothing.md
- [[Proof Verification]] - document - proving-nothing.md
- [[Prover-Killer Attack]] - document - proving-nothing.md
- [[Rollup Stage 012 Decentralization]] - document - proving-nothing.md
- [[SHARP (Shared Prover)]] - document - proving-nothing.md
- [[Scroll (zkEVM, $748M TVL, halo2KZG)]] - document - proving-nothing.md
- [[Starknet (Stwo Circle STARK prover, Cairo ISA)]] - document - proving-nothing.md
- [[Succinct SP1 Hypercube Prover]] - document - proving-nothing.md
- [[Three Converging Forces (privacy, scaling, cost)]] - document - proving-nothing.md
- [[Tornado Cash Governance Attack (2023)]] - document - proving-nothing.md
- [[Trust Minimization (Not Trustless)]] - document - proving-nothing.md
- [[Trusted Setup Ceremony_1]] - document - proving-nothing.md
- [[Upgradeable Proxy Pattern]] - document - proving-nothing.md
- [[Verification Gas Cost]] - document - proving-nothing.md
- [[Verifier Ossification  Immutability]] - document - proving-nothing.md
- [[XZ Utils Supply-Chain Attack (CVE-2024-3094)]] - document - proving-nothing.md
- [[ZK Rollup]] - document - proving-nothing.md
- [[ZKsync Era (Matter Labs, Airbender prover)]] - document - proving-nothing.md
- [[eIDAS 2.0]] - document - proving-nothing.md

## Live Query (requires Dataview plugin)

```dataview
TABLE source_file, type FROM #community/Verification__Rollup_Economics
SORT file.name ASC
```

## Connections to other communities
- 4 edges to [[_COMMUNITY_Proof Systems & Trusted Setup]]
- 2 edges to [[_COMMUNITY_Midnight & Private Contracts]]
- 1 edge to [[_COMMUNITY_Fiat-Shamir Security & Synthesis]]
- 1 edge to [[_COMMUNITY_Market Coprocessors & ZKML]]
- 1 edge to [[_COMMUNITY_ZK Foundations & Framing]]
- 1 edge to [[_COMMUNITY_Privacy-Enhancing Technologies]]

## Top bridge nodes
- [[eIDAS 2.0]] - degree 3, connects to 2 communities
- [[ZK Rollup]] - degree 13, connects to 1 community
- [[On-Chain Verifier]] - degree 7, connects to 1 community
- [[Governance Attack Surface]] - degree 7, connects to 1 community
- [[Verification Gas Cost]] - degree 4, connects to 1 community