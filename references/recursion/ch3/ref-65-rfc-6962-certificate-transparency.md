...
chapters: [3]
type: web
source_url: https://www.rfc-editor.org/rfc/rfc6962
fetched: 2026-06-13
fetched_with: fetcher
citation: 'Laurie, Langley, Kasper, RFC 6962 (Certificate Transparency)'
---

# Laurie, Langley, Kasper, RFC 6962 (Certificate Transparency)

RFC 6962: Certificate Transparency | RFC Editor
...
Privacy Statement
Contact
Your browser has JavaScript disabled so the following tabs will not work. The 'About this RFC' and 'Errata' tabs are instead displayed after the table of contents.
 Contents 
 About this RFC 
 Errata 
3
...
DSS
]) or RSA signatures (RSASSA-PKCS1-
   V1_5 with SHA-256, 
Section 8.2 of [RFC3447]
) using a key of at least
   2048 bits.
...
.  Log Entries
   ...
   signing the resulting TBSCertificate [
RFC5280
] with either
...
RFC 6962
   ...
   protocol version may add new LogEntryType values.  
Section 4
 explains
...
.  Structure of the Signed Certificate Timestamp
   ...
   "tbs_certificate" is the DER-encoded TBSCertificate (see [
RFC5280
])
...
RFC5246
   ...
   "timestamp" is the current NTP Time [
RFC5905
], measured since the
...
3.3
.  Including the Signed Certificate Timestamp in the TLS Handshake
...
RFC 6962
   ...
   Request" TLS extension; see [
RFC6066
]), where the response includes
   an OCSP extension with OID 1.3.6.1.4.1.11129.2.4.5 (see [
RFC2560
])
   and body:
...
Section 4
 explains how clients should handle unknown leaf types.
...
.  Signed Tree Head
   ...
   corresponding Signed Tree Head client message in 
Section 4.3
).  The
...
.  Log Client Messages
   ...
   (JSON) objects [
RFC4627
].  Parameters for GETs are encoded as order-
...
.  Retrieve Merkle Audit Proof from Log by Leaf Hash
   ...
   The "hash" must be calculated as defined in 
Section 3.4
.  The
...
.  Retrieve Entries from Log
   ...
   "tree_size" as returned by "get-sth" in 
Section 4.3
.
...
Section 4.6
   ...
   5.  Fetch the current STH (
Section 4.3
).  Repeat until the STH
       changes.
   ...
   7.  Fetch all the new entries in the tree corresponding to the STH
       (
Section 4.6
).  If they remain unavailable for an extended
...
.  IANA Considerations
   ...
   IANA has allocated an 
RFC 5246
 ExtensionType value (18) for the SCT
